...
fn aeskeygenassist(v1: u128, v2: u8) -> u128 {
    ...
    let y1 = rotword(y0) ^ ((v2 as u32) << 24);
    let y2 = aes_subword(x3);
    let y3 = rotword(y2) ^ ((v2 as u32) << 24);
...
fn mix_column(c: usize, state: u128) -> u32 {
    let i0 = c * 4;
...
fn mix_columns(state: u128) -> u128 {
    let c0 = mix_column(0, state);
    let c1 = mix_column(1, state);
    let c2 = mix_column(2, state);
    let c3 = mix_column(3, state);
    rebuild_u128(c0, c1, c2, c3)
}
...
fn print_num (num : u128) {
    for j in 0..4 {
        for i in 0..4 {
            print!("{:X} ", index_u8(index_u32(num, i), j));
        }
        println!();
    }
    println!();
...
fn test_aeskeygenassist() {
    ...
    let rhs = 0x34e4b524e5b52434018a84eb8b84eb01u128;
    print_num(lhs);
    print_num(rhs);
    assert_eq!(lhs, rhs);
}
...
fn test_key_combine() {
    let key = 0x2b7e151628aed2a6abf7158809cf4f3cu128;
    let ass = 0x34e4b524e5b52434018a84eb8b84eb01u128;
    let rhs = 0xa0fafe1788542cb123a339392a6c7605u128;

    let (lhs, temp2) = key_combine(key, ass, 0);
    print_num(lhs);
...
fn test_aes() {
    ...
    print_num(m_col);
    println!("r_key {:X} vs {:X} is {}", keys_expand(key)[1], 0x046681e5e0cb199a48f8d37a2806264cu128 ^ 0xa49c7ff2689f352b6b5bea43026a5049u128, keys_expand(key)[1] == 0xa0fafe1788242cb123a339392a6c7605u128);
    print_num(keys_expand(key)[1]);
    let k_sch = m_col ^ keys_expand(key)[1];
    println!("k_sch {:X} vs {:X} is {}", k_sch, 0xa49c7ff2689f352b6b5bea43026a5049u128, k_sch == 0xa49c7ff2689f352b6b5bea43026a5049u128);
    print_num(k_sch);
